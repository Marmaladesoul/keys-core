...
| `record_kdbx_state_signature(path: &Path) -> ()` | post-migration | Stat `path`, persist the resulting `(mtime_ms, byte_count)` as the signature corresponding to the engine's current SQLite state. Call after a successful `ingest_from_kdbx`. `save_to_kdbx` calls it automatically. |
